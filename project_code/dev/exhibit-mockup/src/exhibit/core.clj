...
;; circular truss, 1200mm radius, 50mm round cross section
(def truss
  (-> (torus-mesh 1.175 1.225 12 40)
      (g/transform (g/translate M44 [0 -0.5 3.025]))))

;; cross bar for truss support, 2400mm length
(def crossbar
...
(def lcds
  (->> (for [i (range 2)
         ...
         (-> (pl/plane (g/- V3Y) -1.699)
             (g/as-mesh {:p (vec3 (* x 0.325) 0 (+ 0.8 (* i 0.325))) :width 0.1 :height 0.03})))
       (reduce g/into)))
